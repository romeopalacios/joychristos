...
const drawSignature=()=>{
  ...
  const points=[];routes.forEach(route=>{for(let i=1;i<route.length;i++){const a=route[i-1],b=route[i],distance=Math.hypot(b[0]-a[0],b[1]-a[1]),steps=Math.max(2,Math.ceil(distance/4));for(let n=0;n<steps;n++){const t=n/steps;points.push([a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t])}}});
  const brushRadius=34;
  const paint=point=>{const x=point[0]-brushRadius,y=point[1]-brushRadius,size=brushRadius*2;ctx.save();ctx.beginPath();ctx.arc(point[0],point[1],brushRadius,0,Math.PI*2);ctx.clip();ctx.drawImage(image,x,y,size,size,x,y,size,size);ctx.restore()};
